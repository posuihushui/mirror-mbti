...
test.describe("analytics", () => {
  test("home sends a redacted page view and counts the start CTA", async ({ page }) => {
    await page.goto("/?utm_source=e2e&unlock=1");
    const origin = new URL(page.url()).origin;
    await expectEvent(page, "page_view", { page_type: "home", site_language: "zh", page_location: `${origin}/?utm_source=e2e` });
    ...
    await expectEvent(page, "page_view", { page_type: "quiz", page_referrer: `${origin}/?utm_source=e2e` });
  });
  ...
  test("the paid funnel is measured end to end without result IDs or order numbers", async ({ page }) => {
    ...
    await expectEvent(page, "page_view", { page_type: "result", page_location: `${origin}/result/[id]` });
    ...
    await expectEvent(page, "page_view", { page_type: "report", page_location: `${origin}/report/[id]` });
