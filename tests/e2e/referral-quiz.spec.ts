...
async function invitationForHost(request: APIRequestContext) {
  ...
  const inviteResponse = await request.post("/api/comparison-invitations", { headers: { origin }, data: { shareId: share.id, relationship: "friend", consentVersion: "compare-host-v4", requestId: randomUUID() } });
