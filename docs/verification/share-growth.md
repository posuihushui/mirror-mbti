...
## 2026-09-16：本地说明书入口 503 修复

- 本轮起始工作树干净。用户反馈原有 3000 服务的 `GET /api/results/[id]/share-options` 失败；使用其提供的会话复现 `503 SHARE_UNAVAILABLE`，结果归属校验正常。未记录会话 cookie、结果 ID 或结果内容。
- 已核对服务工作目录及 `.env`：连接已确认的本地 `localhost:5432/mirror`。该库仅执行至迁移 0003，分享所需六张表均不存在；首轮实施只迁移了下文记录的隔离测试库，因此原服务仍缺少 0004。
- 修复 `src/db/migrate.ts`：使用安装版 Next.js 文档推荐的 `@next/env` 读取 `.env*`，默认采用开发环境，生产环境可设置 `NODE_ENV=production`，保留显式环境变量的优先级。将 Next 已有的同版本包声明为直接开发依赖，并补充 README 更新后的迁移说明。修复前在未导出 DATABASE_URL 时确认脚本报 `DATABASE_URL is not set`。
- 在上述本地库执行已有 0004 迁移，仅新增六张表及对应约束、索引，没有修改原 results/orders 表。再次运行迁移成功，迁移记录共五条，六张新表均存在。
- 原请求修复后 `200 / ok: true`，返回四条候选、默认三条选择。没有为用户创建公开快照；用户可继续在预览里确认公开内容。
- 本轮验证：typecheck、lint、229 项单测、`git diff --check` 通过。本轮没有页面或业务逻辑改动，未重跑 build、浏览器 e2e 或截图；上一轮完整验收记录保留如下。未提交、未部署。
