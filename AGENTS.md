...
## Mirror Product Direction

- User-requested direction: a Chinese MBTI-style exploration website, designed primarily for WeChat mobile use and also adapted to PC.
- Preserve the supplied reference's cool gray background, black editorial report, photographic side profile, thin radar chart, and pill-shaped primary actions.
- The detailed report is paid; the current ¥6.9 amount is a provisional design assumption, not a confirmed commercial price.
- Keep demo payment clearly labeled until real merchant/order services are provided. Preserve the free test and brief result before the paywall.
- Keep both the original mobile device preview and the standalone responsive `/web.html` entry.
