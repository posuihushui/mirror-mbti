...
import { COMPARE_CONTENT_VERSION, COMPARE_HOST_CONSENT_VERSION, COMPARE_GUEST_CONSENT_VERSION, type CompareSnapshot } from "@/lib/compare-types";
...
function invitationView(row: Invitation) {
  return { id: row.id, token: row.token, url: `${appUrl()}${href(row.locale as Locale, `/t/${row.token}`)}`, locale: row.locale as Locale,
    resultId: row.resultId, shareId: row.shareId, accessPolicy: row.accessPolicy, snapshot: row.publicSnapshot,
...
function pairView(row: Pair, visitorId: string) {
  return { id: row.id, url: href(row.locale as Locale, `/compare/${row.id}`), locale: row.locale as Locale,
    hostSnapshot: row.hostSnapshot, guestSnapshot: row.guestSnapshot, outputSnapshot: row.outputSnapshot, accessPolicy: row.accessPolicy,
...
export async function invitationOptions(resultId: string | undefined, visitorId: string, shareId?: string) {
  ...
  const [active] = await db().select({ invitation: I }).from(I).leftJoin(S, eq(S.id, I.shareId)).where(and(eq(I.visitorId, visitorId), eq(I.resultId, result.id), eq(I.accessPolicy, "paid-pair-v2"), isNull(I.revokedAt), invitationParentOpen, gt(I.expiresAt, new Date()))).limit(1);
  return { resultId: result.id, locale: result.locale, snapshot: result.snapshot, activeInvitation: active ? invitationView(active.invitation) : null };
}
...
export async function createComparisonInvitation(visitorId: string, raw: z.infer<typeof invitationInputSchema>) {
  ...
  return db().transaction(async tx => {
    ...
    const [active] = await tx.select({ id: I.id }).from(I).leftJoin(S, eq(S.id, I.shareId)).where(and(eq(I.visitorId, visitorId), eq(I.resultId, resultId), eq(I.accessPolicy, "paid-pair-v2"), isNull(I.revokedAt), invitationParentOpen, gt(I.expiresAt, new Date()))).limit(1);
...
export async function joinComparison(visitorId: string, raw: z.infer<typeof comparisonInputSchema>) {
  ...
  return db().transaction(async tx => {
    ...
    if (!result) throw new ShareError(404, "NOT_FOUND");
    const guestSnapshot = snapshotFromResult(result), locale = invitation.locale as Locale;
