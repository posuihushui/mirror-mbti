/**
 * Site locales. Chinese is the default and keeps unprefixed URLs (served from `app/[lang]`
 * as `zh` through the proxy rewrite); every other locale lives under its own prefix.
 */
...
export const publishedLocales: readonly Locale[] = ["zh", "en"];
...
/** `href("en", "/quiz")` → `/en/quiz`; `href("zh", "/quiz")` → `/quiz`. */
