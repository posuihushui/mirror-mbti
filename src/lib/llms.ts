 ...
 */
export type LlmsContext = { baseUrl: string; priceLabel: string };
...
const copy = {
  zh: {
    title: site.name,
    summary: (price: string) => `> ${site.description}完整报告 ¥${price} / 次。`,
    facts: (price: string) => [
      `费用：测试、人格类型与简短概览免费；完整报告 ¥${price} 一次性解锁，无订阅、无自动续费。`,
  ...
  en: {
    title: enSite.name,
    summary: (price: string) => `> ${enSite.description} The full report is $${price} once, paid in USDT or USDC.`,
    facts: (price: string) => [
      `Pricing: the test, your type and a short overview are free; the full report is a one-time $${price} unlock paid in USDT or USDC on Ethereum or Solana. No subscription, no auto-renewal.`,
    ...
    pages: [
      ["/pairing", pairingMessages.en.title, pairingMessages.en.summary],
      ...
      ["/report/sample", "Full sample report", "All four chapters in the same layout as the paid report, free to read"],
      ...
      ["/about", "About the test & FAQ", "Versions, how to answer, pricing and the limits of results"],
...
export function llmsText(ctx: LlmsContext, locale: Locale = "zh"): string {
  ...
  return [
    `# ${t.title}`,
    "",
    t.summary(ctx.priceLabel),
    "",
    overview,
    "",
    `## ${t.headings.facts}`,
    "",
    ...t.facts(ctx.priceLabel).map((fact) => `- ${fact}`),
    "",
    `## ${t.headings.pages}`,
    "",
...
export function llmsFullText(ctx: LlmsContext, locale: Locale = "zh"): string {
  ...
  return [
    ...
    `## ${t.headings.faq}`,
    "",
    ...faqsFor(locale, ctx.priceLabel).flatMap(([q, a]) => [`### ${q}`, "", a, ""]),
