...
import { COMPARE_HOST_CONSENT_VERSION, COMPARE_RELATIONSHIPS, HOST_NOTE_MAX } from "@/lib/compare-types";
...
export const relationshipSchema = z.enum(COMPARE_RELATIONSHIPS);
/**
 * Older consent versions still parse so the server can answer CONSENT_REFRESH_REQUIRED. The current
 * version names the relationship, which the invitation publishes; a note needs the current version.
 */
export const invitationInputSchema = z.object({ resultId: pairingResultIdSchema.optional(), shareId: z.uuid().optional(), hostNote: hostNoteSchema.optional(), relationship: relationshipSchema.optional(), consentVersion: z.enum(["compare-host-v1", "compare-host-v2", "compare-host-v3", "compare-host-v4"]), requestId: z.uuid() }).strict().refine(input => input.resultId || input.shareId)
  .refine(input => !input.hostNote || input.consentVersion === COMPARE_HOST_CONSENT_VERSION)
  .refine(input => (input.consentVersion === COMPARE_HOST_CONSENT_VERSION) === Boolean(input.relationship));
/** 请 TA: move one of the host's unused gifts onto an open invitation of theirs. */
export const giftAttachSchema = z.object({ invitationId: z.uuid() }).strict();
