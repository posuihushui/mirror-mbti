...
export async function renderShareImage(snapshot: PublicShareSnapshot, publicUrl: string, format: "portrait" | "og" = "portrait") {
  ...
  return new ImageResponse(<div style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", padding: portrait ? 64 : 44, background: "#edf2f3", color: "#171b1c", fontFamily: OG_FONT_FAMILY }}>
    ...
    {portrait ? <div style={{ display: "flex", flexDirection: "column", width: 832, flex: 1 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", flex: 1, marginTop: 16 }}>{snapshot.lines.map((line, index) => <div key={index} style={{ display: "flex", alignItems: "center", flex: 1, minHeight: 132, gap: 24, borderTop: "1px solid #ccd5d7" }}><div style={{ fontSize: 20, color: "#c49473", width: 32, flexShrink: 0 }}>{String(index + 1).padStart(2, "0")}</div><div style={{ fontSize: en ? 32 : 38, lineHeight: 1.5, flex: 1 }}>{line}</div></div>)}</div>
      {hasOptional && <div style={{ display: "flex", marginTop: 20, flexShrink: 0 }}>{optional}</div>}
      <div style={{ display: "flex", marginTop: 28, height: 196, flexShrink: 0, borderTop: "1px solid #ccd5d7", paddingTop: 24, justifyContent: "space-between", gap: 36, alignItems: "center" }}><div style={{ display: "flex", flex: 1, flexDirection: "column", gap: 16 }}><div style={{ fontSize: 24, lineHeight: 1.5 }}>{snapshot.disclaimer}</div><div style={{ fontSize: 16, lineHeight: 1.5, color: "#627176" }}>{t.testNote}</div><div style={{ fontSize: 17 }}>{t.scan}</div></div>{qr && <img src={qr} alt="" width={160} height={160} />}</div>
