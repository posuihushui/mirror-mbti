...
export default async function HelpPage() {
  ...
  return <><AppHeader variant="page" title={t.headerTitle} backHref={href(locale, "/")} path="/help" />
    <main className="mx-auto max-w-[760px] px-[27px] pt-7 pb-20 md:px-10 md:pt-[55px]">
      ...
      <section className="mt-7 border-t border-line pt-6"><h2 className="text-[20px]">{shareHelpMessages[locale].title}</h2><p className="mt-4 text-[13px] leading-[2] text-mist">{shareHelpMessages[locale].body}</p><Link href={href(locale,"/my/shares")} prefetch={false} className="text-link mt-4">{locale === "en" ? "My shares" : "我的分享"}</Link></section>
