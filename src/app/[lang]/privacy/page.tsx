...
export default async function PrivacyPage() {
  ...
  if (locale === "en") {
    return (
      <LegalPage eyebrow={pageMessages.en.legal.eyebrow} path="/privacy" title={copy.en.title} updated={copy.en.updated}>
        <PrivacyEn />
      </LegalPage>
    );
  }
  return (
    <LegalPage eyebrow={pageMessages.zh.legal.eyebrow} path="/privacy" title={copy.zh.title} updated={copy.zh.updated}>
      <h2>我们收集什么</h2>
      ...
      <h2>Cookie 与本地存储</h2>
      <p>访客标识 Cookie 有效期为一年，仅用于识别浏览器。各版本作答进度、题目顺序、最近一次结果编号，以及报告读到的章节与七天练习的打卡，会保存在浏览器本地存储中，清除浏览器数据会一并清除；服务器上的结果不受影响，但在没有 Cookie 的浏览器中将无法自动找回。</p>
      ...
      <h2>关于本测试</h2>
      <p>题目为独立原创的问卷，并非官方 MBTI 量表，也未经过心理测量学验证。结果仅供自我探索，不构成任何心理诊断或专业建议。</p>
    </LegalPage>
  );
}
