...
export default async function ComparisonPage({ params }: Props) {
  ...
  // A guide written for a relationship says which one in its heading.
  const title = pair.relationship ? m.titleFor(m.relationshipLabels[pair.relationship]) : m.title;
  return <><AppHeader variant="page" title={m.title} backHref={href(locale, "/my/pairing")} /><main data-share-static className="mx-auto max-w-5xl px-6 py-8 md:py-14"><h1 className="mb-8 text-3xl leading-tight md:text-4xl">{title}</h1><ComparisonReadingActions id={id} locale={locale}><ComparisonVisit pairId={id} /><ComparisonReading content={pair.outputSnapshot} locale={locale} sides={{ you: (pair.role === "host" ? pair.hostSnapshot : pair.guestSnapshot).categories, other: (pair.role === "host" ? pair.guestSnapshot : pair.hostSnapshot).categories, youLabel: m.you, otherLabel: m.other }} /><section className="mt-10 border-t border-line pt-7"><div className="flex flex-col gap-4 md:flex-row md:items-center"><PrimaryButton href={href(locale, "/my/pairing")} prefetch={false} className="md:w-auto md:min-w-64">{m.inviteAnother}</PrimaryButton><TextLink href={href(locale, `/compare/${id}/image`)} prefetch={false} className="shrink-0">{m.saveImage}</TextLink></div><p className="mt-4 text-xs text-mist">{m.saveImageNote}</p></section><section className="mt-9 border-t border-line pt-7"><h2 className="eyebrow text-mist">{m.sharedScope}</h2><div className="mt-4"><CompareReveal mode="panels"><div data-compare-motion="host"><PreferenceSummary snapshot={pair.hostSnapshot} locale={locale} title={pair.role === "host" ? m.you : m.other} /></div><span data-compare-motion="dot" aria-hidden="true" className={styles.dot} /><div data-compare-motion="guest"><PreferenceSummary snapshot={pair.guestSnapshot} locale={locale} title={pair.role === "guest" ? m.you : m.other} /></div></CompareReveal></div></section></ComparisonReadingActions></main></>;
}
