...
export default async function ComparisonPage({ params }: Props) {
  ...
  return <><AppHeader variant="page" title={m.title} backHref={href(locale, "/my/pairing")} /><main data-share-static className="mx-auto max-w-[960px] px-6 py-8 md:py-14"><h1 className="mb-8 text-[28px] leading-[1.3]">{m.title}</h1><ComparisonReadingActions id={id} locale={locale}><ComparisonVisit pairId={id} /><CompareReveal mode="panels"><div data-compare-motion="host"><PreferenceSummary snapshot={pair.hostSnapshot} locale={locale} title={pair.role === "host" ? m.you : m.other} /></div><span data-compare-motion="dot" aria-hidden="true" className={styles.dot} /><div data-compare-motion="guest"><PreferenceSummary snapshot={pair.guestSnapshot} locale={locale} title={pair.role === "guest" ? m.you : m.other} /></div></CompareReveal><ComparisonReading content={pair.outputSnapshot} locale={locale} /></ComparisonReadingActions></main></>;
}
