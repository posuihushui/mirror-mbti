  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-6xl px-6 pt-6 pb-[135px] md:px-10 md:pt-14 md:pb-20">
        ...
        <section className="mt-10 md:mt-14" aria-labelledby="type-map">
          <h2 id="type-map" className="text-xl">{t.mapHeading}</h2>
          <div className="mt-5"><TypeMap locale={locale} location="type_grid" /></div>
        </section>
        <h2 className="mt-14 text-xl md:mt-20">{t.groupsHeading}</h2>
        {t.groups.map(([letters, groupName, description]) => (
          <section key={letters} className="mt-8 md:mt-10" aria-labelledby={`group-${letters}`}>
            <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1 border-b border-line pb-3">
              <h3 id={`group-${letters}`} className="text-lg">{groupName} <span className="ml-1 text-sm font-normal text-mist">{letters}</span></h3>
              <p className="text-sm text-mist">{description}</p>
            </div>
            <ul className="mt-4 grid list-none gap-3 p-0 md:grid-cols-4 md:gap-4">
              {TYPES.filter((type) => temperament(type) === letters).map((type) => {
