...
export default async function MyShares(){const locale=await getLocale();const t=shareMessages[locale];const visitor=await getVisitorId();const list=visitor?await listOwnedShares(visitor):{items:[],nextCursor:null};return <><AppHeader variant="page" title={t.myShares} backHref={href(locale,"/my/report")} path="/my/shares"/><main data-share-static className="mx-auto max-w-[1000px] px-[22px] py-8 md:px-10 md:py-14"><h1 className="mb-7 text-3xl">{t.myShares}</h1><TextLink href={href(locale,"/my/pairing")} prefetch={false} className="mb-7">{pairingUiMessages[locale].center}</TextLink><ShareManager {...list} locale={locale}/></main></>;}
