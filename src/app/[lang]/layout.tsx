...
import { href, htmlLang, isPublishedLocale, type Locale, ogLocale, publishedLocales } from "@/lib/i18n/locale";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    // Route handlers serve each manifest, so this link follows the page's language.
    ...
    openGraph: {
      ...
      title: copy.title,
      description: copy.description,
      images: [{ url: href(locale, "/opengraph-image"), width: 1200, height: 630, alt: `${copy.name} — ${copy.tagline}` }],
    },
    twitter: { card: "summary_large_image", title: copy.title, description: copy.description, images: [{ url: href(locale, "/opengraph-image"), width: 1200, height: 630, alt: `${copy.name} — ${copy.tagline}` }] },
 ...
 * site 观己 mirror and described it in Chinese would tell crawlers the English root is a Chinese site.
...
function organizationJsonLd(locale: Locale) {
  ...
  return {
    ...
    ...(locale === "zh" ? { alternateName: [site.brandZh, site.brand] } : {}),
    url: APP_URL,
...
/** Root layout for every page. English reaches it as `en` through the proxy rewrite, so its URLs stay unprefixed. */
