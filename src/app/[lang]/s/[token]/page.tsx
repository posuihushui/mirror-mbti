...
export default async function PublicShare({params}: Props) {
 ...
 return <><AppHeader variant="page" title={t.title} backHref={href(locale,"/")} /><main data-share-static className="mx-auto max-w-[1060px] px-[22px] py-8 md:px-10 md:py-14"><div className="grid gap-8 md:grid-cols-[minmax(0,520px)_1fr] md:items-center md:gap-14"><ShareCard snapshot={share.snapshot}/><section><h1 className="text-3xl md:text-4xl">{t.publicHeading}</h1><p className="mt-4 text-sm text-mist">{t.publicDescription}</p><ShareQuizLink token={token} locale={locale} href={href(locale,"/quiz")} className="pill mt-7 flex">{t.start}</ShareQuizLink><p className="mt-4 text-xs text-mist">{t.freeNote}</p><TextLink className="mt-3" href={href(locale,"/help")}>{t.help}</TextLink><div className="mt-9 border-t border-line pt-7"><p className="eyebrow text-mist">{t.pairTitle}</p><p className="mt-3 text-sm text-mist">{t.pairBody}</p><TextLink className="mt-2" href={href(locale,"/pairing")}>{t.pairLink}</TextLink></div></section></div><ShareVisit token={token} locale={locale}/>{env().WECHAT_SHARE_ENABLED === "true" && <WeChatShare title={t.title} desc={t.publicDescription} link={appUrl()+href(locale,`/s/${token}`)} imgUrl={appUrl()+href(locale,`/s/${token}/opengraph-image`)} />}</main></>;
}
