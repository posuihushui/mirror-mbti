...
export function PrivacyEn() {
  return (
    <>
      ...
      <p>When you buy a full report, we record the order number, amount, currency, payment status and the payment reference from whichever checkout processed it. Card payments run on Waffo Pancake, the merchant of record: you enter your email, billing details and card on its checkout rather than here, so we never receive or store your card number, and it keeps its own records as the seller. For payments in USDT or USDC we also record the network, token and the blockchain transaction hash; for Ethereum we record the wallet address you connect and a signed message proving you control it; signing that message never moves funds. Blockchain transactions are public and permanent by design, and we cannot change or remove data on a public ledger. We will never ask for your private keys or recovery phrase.</p>
      ...
      <p>The visitor-ID cookie lasts one year and only identifies your browser. Progress for each version, question order, your most recent result ID, the chapter you last read in a report and your seven-day practice check-ins are kept in your browser’s local storage; clearing browser data removes them. Results on our server are unaffected, but a browser without the cookie can’t find them automatically.</p>
      ...
      <p>Results and order information are kept while the service runs. To delete data related to you, email <a href={`mailto:${site.supportEmail}`}>{site.supportEmail}</a> with a result ID or order number, and we will handle it after verifying ownership. Records held by the payment provider as seller of record, and on-chain payment records, are outside our systems and can’t be deleted by us.</p>
