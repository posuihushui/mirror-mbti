...
export function PreferenceSummary({ snapshot, locale, title }: { snapshot: CompareSnapshot; locale: Locale; title: string }) {
  ...
  return <section className="min-w-0 rounded-[4px] border border-line bg-card p-5">
    ...
    <ul className="mt-4 flex flex-wrap gap-x-5 gap-y-2">{(["EI", "SN", "TF", "JP"] as const).map((dimension) => <li key={dimension} className="flex items-baseline gap-2 text-sm leading-[1.6]">
      <span aria-hidden="true" className="text-[11px] tracking-[.06em] text-mist">{dimension}</span>{m.categoryLabels[snapshot.categories[dimension]]}
    </li>)}</ul>
    <dl className="mt-4 flex flex-wrap gap-x-5 gap-y-1 border-t border-line pt-3 text-[11px] leading-[1.8] text-mist">
