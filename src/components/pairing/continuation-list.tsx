...
export function ContinuationList({ items, locale, surface = "result" }: { items: ContinuationItem[]; locale: Locale; surface?: "result" | "report" | "my_pairing" | "payment_sheet" | "pay_status" }) {
  ...
  return <section data-pairing-continuations className="my-6 border border-line p-5"><h2 className="text-xl leading-[1.5]">{rows.length > 1 ? m.continueChoice : m.continue}</h2><p className="mt-3 text-xs leading-[1.8] text-mist">{m.continuationSaved}</p><ul className="mt-4 space-y-4">{rows.map((item, index) => <li key={item.id} className="flex flex-wrap items-center justify-between gap-3"><a href={item.continueUrl} className="pill min-h-11 text-sm" onClick={() => emitPairingResume(item.id, surface)}>{m.continue}{rows.length > 1 ? ` · ${index + 1}` : ""}</a><button type="button" className="text-link min-h-11 text-xs" disabled={pending !== null} onClick={() => cancel(item.id)}>{s.cancel}</button></li>)}</ul><p role="status" className="text-xs">{error}</p></section>;
}
