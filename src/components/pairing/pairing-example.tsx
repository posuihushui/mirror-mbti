...
export function PairingExample({ locale }: { locale: Locale }) {
  ...
  return <section data-pairing-example className="border border-line bg-card p-5 md:p-7">
    ...
    <PairingReveal stagger><div className="mt-5 bg-[#ead9cc] p-4 text-sm leading-[1.8]"><p className="text-xs text-mist">{m.firstPerson}</p><p>{m.firstLine}</p></div><div className="mt-3 border-l border-line pl-4 text-sm leading-[1.8]"><p className="text-xs text-mist">{m.secondPerson}</p><p>{m.secondLine}</p></div></PairingReveal>
