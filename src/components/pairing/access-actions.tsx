...
type Access = { eligibility: "eligible" | "locked" | "unavailable" | "syncing"; continuations: ContinuationItem[] };
export function AccessActions({ resultId, locale, surface, onReady }: { resultId: string; locale: Locale; surface: "payment_sheet" | "pay_status" | "result"; onReady?: () => void }) {
  ...
  if (access?.eligibility === "eligible") return <div data-pairing-access="eligible" className="mt-5"><p role="status" className={`${styles.status} text-sm leading-[1.8]`}>{m.ready}</p><p className="mt-2 text-xs leading-[1.8] text-mist">{m.readyRules}</p><ContinuationList items={access.continuations} locale={locale} surface={surface} /><a href={href(locale, `/report/${resultId}`)} className={`${access.continuations.length ? "text-link" : "pill"} mt-5 flex min-h-11 justify-center`}>{m.readReport}</a><a href={href(locale, `/my/pairing?result=${resultId}`)} onClick={() => emitPairingEvent("pairing_entry_clicked", resultId, surface)} className="text-link mt-3 inline-flex min-h-11 text-sm">{m.invite}</a></div>;
